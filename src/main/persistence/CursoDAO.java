...
public class CursoDAO extends SQLiteOpenHelper {
    ...
    private void regenerateDB(SQLiteDatabase db)
    {
	...
	Curso findCursoById(int id) {
		...
		Cursor cursor = db.query(TABLE_CURSOS, new String[] { KEY_ID,
				KEY_ANIO, KEY_CUATRI, KEY_LETRA }, KEY_ID + "=?",
	...
	Curso findCursoByAnioCuatriLetra(String anio, String cuatri, String letra) {
        SQLiteDatabase db = this.getReadableDatabase();

        Cursor cursor = db.query(TABLE_CURSOS, new String[] { KEY_ID,
                KEY_ANIO, KEY_CUATRI, KEY_LETRA }, String.format("%s = ?, %s = ?, %s = ?", KEY_ANIO, KEY_CUATRI, KEY_LETRA),
                new String[] {anio, cuatri, letra}, null, null, null, null);
        if (cursor != null)
            cursor.moveToFirst();
